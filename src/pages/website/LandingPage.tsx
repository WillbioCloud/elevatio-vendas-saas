...
// ============================================================
// ELEVATIO VENDAS — Landing Page SaaS (GSAP Otimizado)
// ============================================================

const PLANS = [
  {
    name: 'Starter', price: 54.90, desc: 'Ideal para corretores independentes que estão começando.', highlight: false,
    users: '2', properties: '50', ia: '50/mês', pipeline: false, gamification: false, erp: false, portals: false,
    features: ['Até 2 usuários', 'Até 50 imóveis', '50 descrições com IA/mês', 'CRM Básico'],
  },
  {
    name: 'Basic', price: 74.90, desc: 'Para pequenas imobiliárias com foco em crescimento.', highlight: false,
    users: '5', properties: '400', ia: '200/mês', pipeline: true, gamification: false, erp: false, portals: false,
    features: ['Até 5 usuários', 'Até 400 imóveis', 'Pipeline de Leads', 'Gestão de Tarefas'],
  },
  {
    name: 'Profissional', price: 119.90, desc: 'O padrão da indústria para imobiliárias consolidadas.', highlight: true, badge: 'Mais popular',
    users: '8', properties: '1.000', ia: '600/mês', pipeline: true, gamification: true, erp: false, portals: false,
    features: ['Até 8 usuários', 'Até 1.000 imóveis', 'Gamificação', 'Relatórios Avançados'],
  },
  {
    name: 'Business', price: 179.90, desc: 'Para quem precisa de controle total e automação.', highlight: false,
    users: '12', properties: '2.000', ia: '1.000/mês', pipeline: true, gamification: true, erp: true, portals: false,
    features: ['Até 12 usuários', 'Até 2.000 imóveis', 'Contratos e Finanças', 'Automação de Marketing'],
  },
  {
    name: 'Premium', price: 249.90, desc: 'Tecnologia de ponta com IA para alta performance.', highlight: false,
    users: '20', properties: '3.500', ia: '1.450/mês', pipeline: true, gamification: true, erp: true, portals: true,
    features: ['Até 20 usuários', 'Até 3.500 imóveis', 'Aura AI (Assistente)', 'Integração de Portais'],
  },
  {
    name: 'Elite', price: 349.90, desc: 'Sem limites. Para os maiores players do mercado.', highlight: false,
    users: 'Ilimitado', properties: 'Ilimitado', ia: 'Ilimitada', pipeline: true, gamification: true, erp: true, portals: true,
...
const COMPARE_ROWS = [
  ...
  { category: 'IA', label: 'Descrições IA/mês', field: 'ia' },
  { category: 'Módulos', label: 'Pipeline de Leads', field: 'pipeline', bool: true },
  { category: 'Módulos', label: 'Gamificação', field: 'gamification', bool: true },
  { category: 'Módulos', label: 'Contratos e Finanças', field: 'erp', bool: true },
  { category: 'Integrações', label: 'Portais Imobiliários', field: 'portals', bool: true },
];
  ...
  useLayoutEffect(() => {
    const ctx = gsap.context(() => {
      ScrollSmoother.create({
        ...
        smooth: 1.2, // Um pouco mais rápido/natural
        effects: true,
...
// ─────────────────────────────────────────────
// Hook: ScrollTrigger fade-in (Mais rápido)
// ─────────────────────────────────────────────
  ...
  useLayoutEffect(() => {
    ...
    const ctx = gsap.context(() => {
      gsap.from(targets, {
        ...
        scrollTrigger: { 
          trigger: ref.current!, 
          start: 'top 92%', // Ativa mais cedo para não dar a sensação de tela vazia
          once: true 
        },
      });
    });
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 32px', height: 72, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <nav style={{ display: 'flex', gap: 36 }} className="ev-nav-desktop">
          {[['Início','#hero'],['Planos','#planos'],['Recursos','#recursos'],['FAQ','#faq']].map(([l,h]) => (
            <a key={l} href={h} className="ev-nav-link" style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 15, fontWeight: 600, textDecoration: 'none', color: linkColor, transition: 'color 0.2s' }}>{l}</a>
          ))}
        </nav>
        <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
          <a href="/admin/login" style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 14, fontWeight: 600, textDecoration: 'none', color: scrolled ? '#374151' : '#ffffff', padding: '8px 16px', transition: 'opacity 0.2s' }} className="hover:opacity-80">Entrar</a>
...
const Hero: React.FC = () => {
  ...
  useLayoutEffect(() => {
    const ctx = gsap.context(() => {
      // Animações entram muito mais rápido agora
...
const Pricing: React.FC = () => {
  ...
  const ref = useRef<HTMLElement>(null);
  useFadeIn(ref, { selector: '.ev-plan', stagger: 0.1, y: 30 });
                ...
                <ul style={{ listStyle: 'none', padding: 0, margin: '0 0 28px 0', display: 'flex', flexDirection: 'column' as const, gap: 10 }}>
                  {plan.features.map((feat, j) => (
                    <li key={j} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" className="shrink-0"><circle cx="12" cy="12" r="10" fill={plan.highlight ? 'rgba(14,165,233,0.2)' : 'rgba(26,86,219,0.08)'}/><path d="M8 12l3 3 5-6" stroke={plan.highlight ? '#7dd3fc' : '#1a56db'} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
                      <span style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 14, color: plan.highlight ? 'rgba(255,255,255,0.9)' : '#475569' }}>{feat}</span>
                    </li>
                  ))}
                </ul>
                <a href="#" className={plan.highlight ? 'ev-btn-primary' : 'ev-btn-light'} style={{ display: 'block', textAlign: 'center', fontFamily: "'DM Sans',sans-serif", fontSize: 15, fontWeight: 700, textDecoration: 'none', padding: '13px 0', borderRadius: 12 }}>Testar grátis</a>
              </div>
            );
          })}
        </div>
  ...
  useLayoutEffect(() => {
    ...
    const ctx = gsap.context(() => {
      gsap.from(tableRef.current!.querySelectorAll('.ev-cmp-row'), {
        opacity: 0, x: -10, duration: 0.4, stagger: 0.03, ease: 'power2.out',
            ...
            {COMPARE_ROWS.map((row, ri) => {
              ...
              return (
                <React.Fragment key={ri}>
                  ...
                  <div className="ev-cmp-row ev-row-hover" style={{ display: 'grid', gridTemplateColumns: '220px repeat(6, 1fr)', borderTop: '1px solid #f1f5f9' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '220px repeat(6, 1fr)', borderTop: '2px solid #e2e8f0', background: '#f8fafc' }}>
              ...
              {PLANS.map((plan, i) => (
                <div key={i} style={{ padding: '16px 8px', borderLeft: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', justifyContent: 'center', background: plan.highlight ? 'rgba(26,86,219,0.03)' : 'transparent' }}>
                  <a href="#" className={plan.highlight ? 'ev-btn-primary' : 'ev-btn-light'} style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 13, fontWeight: 700, textDecoration: 'none', padding: '9px 16px', borderRadius: 10, whiteSpace: 'nowrap' as const }}>
                    Testar
                  </a>
                </div>
              ))}
    ...
    <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 32px' }}>
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr', gap: 40, marginBottom: 48 }}>
        ...
        {[{ title: 'Produto', items: ['Recursos','Planos','Templates','Integrações'] }, { title: 'Empresa', items: ['Sobre','Blog','Parceiros','Contato'] }].map(col => (
          <div key={col.title}>
            ...
            {col.items.map(item => <div key={item} style={{ marginBottom: 10 }}><a href="#" className="hover:text-white" style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 14, color: 'rgba(255,255,255,0.6)', textDecoration: 'none', transition: 'color 0.2s' }}>{item}</a></div>)}
          </div>
        ))}
        <div>
      ...
      <div style={{ borderTop: '1px solid rgba(255,255,255,0.07)', paddingTop: 24, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap' as const, gap: 12 }}>
        ...
        <div style={{ display: 'flex', gap: 24 }}>
          {['Privacidade','Termos'].map(item => <a key={item} href="#" className="hover:text-white" style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 13, color: 'rgba(255,255,255,0.3)', textDecoration: 'none', transition: 'color 0.2s' }}>{item}</a>)}
        </div>
      </div>
    </div>
  </footer>
...
export default function LandingPage() {
  ...
  return (
    <>
      <style>{`
        @import url('[https://fonts.googleapis.com/css2?family=Sora:wght@400;600;700;800;900&family=DM+Sans:ital,wght@0,400;0,500;0,600;0,700;1,400&display=swap](https://fonts.googleapis.com/css2?family=Sora:wght@400;600;700;800;900&family=DM+Sans:ital,wght@0,400;0,500;0,600;0,700;1,400&display=swap)');
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* Classes de Hover e Botões (Dando Vida) */
        .ev-btn-primary {
